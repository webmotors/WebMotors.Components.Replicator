...
{
	public class UpdateAnalizer : QueryAnalizer
	{
		#region [ protected ]
		protected override void Resolve(CoreModel model, Constants constants)
		{
			model.update(QueryFields(model), _container, constants);
		}

		protected override string GetTableName()
		{
			if (string.IsNullOrWhiteSpace(Query) || Query.ToLower().Contains(" join ")) return string.Empty;
			var table = Query.ToLower().Replace("update", string.Empty).Replace("`", string.Empty).Replace("'", string.Empty).Trim().Split(' ')[0];
			if (table.IndexOf(".") > -1)
